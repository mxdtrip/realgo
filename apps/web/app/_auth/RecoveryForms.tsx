...
export function ForgotPasswordForm() {
  ...
  return <section className="auth-panel"><div className="auth-panel__heading"><h1>Сброс пароля</h1><p>Укажи email — отправим ссылку для нового пароля.</p></div>{done ? <p className="auth-form__success">Запрос принят. Если аккаунт с таким адресом существует, мы отправим инструкции. Доставка может занять несколько минут.</p> : <form className="auth-form" onSubmit={submit}><label><span className="auth-field-label">Email</span><input autoComplete="email" required type="email" value={email} onChange={(e) => setEmail(e.target.value)} /></label>{error ? <p className="auth-form__error" role="alert">{error}</p> : null}<button disabled={pending} type="submit">{pending ? "Отправляем…" : "Отправить ссылку"}</button></form>}<p className="auth-panel__switch"><Link href="/login">Вернуться ко входу</Link></p></section>;
}

export function ResetPasswordForm() {
  const router = useRouter(); const [token, setToken] = useState(""); const [password, setPassword] = useState(""); const [repeat, setRepeat] = useState(""); const [error, setError] = useState(""); const [pending, setPending] = useState(false);
  useEffect(() => {
    const url = new URL(window.location.href);
    setToken(new URLSearchParams(url.hash.slice(1)).get("token") || url.searchParams.get("token") || "");
...
export function VerifyEmailForm() {
	...
	async function resend() { if (!email.trim()) { setError("Сначала укажи email."); return; } setPending(true); setError(""); try { await auth.requestEmailVerification(email.trim()); setNotice("Запрос принят. Дождитесь письма и проверьте папку «Спам»."); } catch (e) { setError(errorMessage(e)); } finally { setPending(false); } }
